...
workflow {  
    ...
    //gatk_mark_duplicates.out.dedup_bam.view()
    //gatk_mark_duplicates.out.dedup_index.view()
    ...
    //applybqsr_input.view()
    gatk_applybqsr(applybqsr_input)

    

    if (params.mode == "germline") {
        println "Running in germline mode"
        GERMLINE_PIPELINE(gatk_applybqsr.out.dedup_bqsr_bam, gatk_applybqsr.out.dedup_bqsr_index)
    }
}


workflow GERMLINE_PIPELINE {
    ...
    main:
    gatk_HaplotypeCaller(bam_ch, bai_ch)
   ...
   //Collect gvcf files and their indexes for CombineGVCFs
    ...
    //gatk_combine_gvcfs.out.vcf.view()
